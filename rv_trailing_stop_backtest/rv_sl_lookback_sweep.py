...
All other params are fixed at live bot values:
  rv_period=10, rv_threshold=3.0, ema_period=9,
  min_sl_pct=0.4%, trailing_pct=0.6%
"""
...
# ── Params (fixed at live values) ─────────────────────────────────────────────
RISK_AMOUNT       = 10.0
MARKET_FEE        = 0.0005   # 0.05% per side
FEE_PCT           = 0.1      # used in position sizing (matches live bot)
RV_THRESHOLD      = 3.0
RV_PERIOD         = 10
EMA_PERIOD        = 9
TRAILING_PCT      = 0.60
...
SL_LOOKBACKS = [1, 5, 10, 15]   # candles to look back for rolling low/high
...
CACHE_30M = os.path.join(HERE, "data/SOLUSDT_30m_20230311_20260311.csv")
DATA_15M  = os.path.join(HERE, "../mean_reversion_backtest/data/SOLUSDT_15m_20230311_20260311.csv")


def load_data() -> pd.DataFrame:
    if os.path.exists(CACHE_30M):
        df = pd.read_csv(CACHE_30M)
    else:
        df15 = pd.read_csv(DATA_15M)
        df15["open_time"] = pd.to_datetime(df15["open_time"], utc=True)
        df = df15.set_index("open_time").resample("30min").agg(
...
def summarise(trades: list, sl_lookback: int) -> dict:
    ...
    # SL distance stats (how much wider is rolling SL vs single-candle)
...
if __name__ == "__main__":
    ...
    W = "=" * 80
    print(f"\n\n{W}")
    print("  RV SOL 30m — SL LOOKBACK COMPARISON")
    ...
    # ── Save summary CSV ─────────────────────────────────────────────────────
    out_csv = os.path.join(HERE, "rv_sl_lookback_results.csv")
